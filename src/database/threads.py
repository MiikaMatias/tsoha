...
def get_threads_by_username(username, db):
    ...
    threads.id, threads.owner_id, threads.title, threads.created_at, threads.content, users.username
    FROM threads
    LEFT JOIN (
...
def get_thread_by_id(id, db):
    sql = f"SELECT id, owner_id, title, created_at, content FROM threads WHERE id = (:id) AND show=TRUE;"
